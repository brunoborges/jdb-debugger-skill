...
handoffs:
  - agent: "jdb-session"
    ...
    prompt: "Start an interactive JDB debugging session — launch a JVM under JDB or attach to a running one. Set breakpoints, step through code, inspect variables, and catch exceptions."
  - agent: "jdb-diagnostics"
    label: "Collect diagnostics"
    prompt: "Collect quick diagnostics from a running JVM — thread dumps, deadlock detection, and class listings for a health check without a full interactive debugging session."
  - agent: "jdb-analyst"
...
- Pass all gathered context to the sub-agent
